...
class Dragon extends JComponent {
	...
	private void drawDragon(int rank, Point p1, Point p3, Graphics g) {
		...
		if (rank == 0) { // stopping condition when rank is 0
			g.drawLine(p1.x, p1.y, p3.x, p3.y);
		} else if (rank > 0) { // when rank is greater than 0, call recrusive steps to connect segments
			drawDragon(rank - 1, p1, p2, g);
			drawDragon(rank - 1, p3, p2, g);
		} else {
			throw new IllegalArgumentException("Rank cannot be lower than 0"); // throw exception when rank is below 0
		}
	}
