...
class Snowflake extends JComponent {
	...
	private void drawEdge(int rank, Point p1, Point p5, Graphics g) {
		...
		Point p3 = new Point((Math.toIntExact(Math.round(p1.x + dx / 2 + dy * Math.sqrt(3) / 6))),
				Math.toIntExact(Math.round(p1.y - dx * Math.sqrt(3) / 6 + dy / 2)));
		Point p4 = new Point(p5.x - dx / 3, p5.y - dy / 3);

		// WRITE THE RECURSIVE CODE HERE:
		if (rank != 0) { // when rank is greater than 0, call recrusive steps to connect segments
			drawEdge(rank - 1, p1, p2, g);
			drawEdge(rank - 1, p2, p3, g);
			drawEdge(rank - 1, p3, p4, g);
			drawEdge(rank - 1, p4, p5, g);
		} else if (rank == 0) { // stopping condition when rank is 0
			g.drawLine(p1.x, p1.y, p5.x, p5.y);
		} else {
			throw new IllegalArgumentException("Rank cannot be lower than 0"); // throw exception when rank is below
		}
	}
